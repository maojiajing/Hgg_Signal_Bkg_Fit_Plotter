...
parser.add_argument('-i2', '--inputFile2', dest="dname2", type=str, default=None, required=True)
...
hfile = TFile(opt.dname2, "READ")
ofile = TFile(opt.outf+".root", "RECREATE")

w_all = tfile.Get("MaxLikelihoodFitResult")
hBkg = hfile.Get("shapes_fit_s/bin2/Bkg")
hSMH = hfile.Get("shapes_fit_s/bin2/SMH")
#hsignal = hfile.Get("shapes_fit_s/bin2/signal")
htotal_background = hfile.Get("shapes_fit_s/bin2/total_background")
htotal_signal = hfile.Get("shapes_fit_s/bin2/total_signal")
htotal = hfile.Get("shapes_fit_s/bin2/total")
...
#  sys.exit()
  ...
  htotal_background.SetLineColor(cNiceBlueDark)
  htotal_background.SetLineStyle(kDashed)
  htotal_background.SetFillColorAlpha(cNiceBlueDark,0.35)
  htotal_background.SetFillStyle(4050)

  htotal_signal.SetLineColor(cNiceRed)
  htotal_signal.SetLineStyle(kDashed)
  htotal_signal.SetFillColorAlpha(cNiceRed,0.35)
  htotal_signal.SetFillStyle(4050)
  
  htotal.SetLineColor(cNiceGreenDark)
  htotal.SetLineStyle(kSolid)
  htotal.SetFillColorAlpha(cNiceGreenDark,0.35)
  htotal.SetFillStyle(4050)
...
#  leg.SetNColumns(3)
  ...
  #leg.AddEntry(htotal_background, 'Total background', 'l')
  #leg.AddEntry(htotal_signal, sigText, 'l')
  #leg.AddEntry(htotal, 'Signal plus background', 'l')
  ...
  #hBkg.Draw('pe1same')
  htotal_background.Draw('pe3same')
  htotal_signal.Draw('pe3same')
  htotal.Draw('pe3same')
  c.Update()
  SetPadStyle(c)
  c.Update()
